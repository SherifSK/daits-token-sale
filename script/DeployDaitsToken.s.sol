...
contract DeployDaitsToken is Script {
    ...
    function run() public {
        ...
        DaitsToken daitsToken = new DaitsToken(
            tokenName,
            tokenSymbol,
            multisigAdmin,
            maxSupply
    ...
    function _verifyDeployment(DaitsToken token, address expectedAdmin) internal view {
        ...
        require(
            ...
            keccak256(abi.encodePacked(token.name())) == keccak256(abi.encodePacked(vm.envOr("TOKEN_NAME", DEFAULT_TOKEN_NAME))),
            "Token name mismatch"
        );
        
        require(
            keccak256(abi.encodePacked(token.symbol())) == keccak256(abi.encodePacked(DEFAULT_TOKEN_SYMBOL)) ||
            keccak256(abi.encodePacked(token.symbol())) == keccak256(abi.encodePacked(vm.envOr("TOKEN_SYMBOL", DEFAULT_TOKEN_SYMBOL))),
